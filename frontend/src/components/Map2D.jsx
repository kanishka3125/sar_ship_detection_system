...
function InnerMap({ ships, onSelectShip, focusShip, environment, visible }) {
  ...
  useEffect(() => {
    ...
    validShips.forEach(ship => {
      ...
      <span style="background:${color}15;color:${color};padding:2px 6px;border-radius:4px;font-size:10px;font-weight:700;border:1px solid ${color}30;">${ship.risk || 'LOW'} RISK</span>
      </div>
      ...
      ${((ship.alert_reason || '').length > 90)
        ? (ship.alert_reason || '').slice(0, 90) + '…'
        : (ship.alert_reason || 'No intelligence available')}
    </div>
    ...
    <div style="font-size:10px;color:var(--text-dim);font-family:var(--font-mono);background:var(--bg-secondary);padding:6px;border-radius:4px;">
      <div style="margin-bottom:3px;">
        LAT: ${typeof lat === 'number' ? lat.toFixed(4) : '--'}°N  
        LNG: ${typeof lng === 'number' ? lng.toFixed(4) : '--'}°E
      </div>

      <div>
        SPD: ${ship.speed_knots || 0}kt | 
        HDG: ${ship.heading || 0}° | 
        AIS: <span style="color:${ship.ais_status === 'ABSENT' ? '#d32f2f' : '#388e3c'};font-weight:600;">
          ${ship.ais_status || 'UNKNOWN'}
        </span>
      </div>
    </div>
    ...
    ${weatherInfo}
      ...
      // Add hover tooltip
      marker.bindTooltip(`<b>${ship.vessel_name || 'Unknown'}</b><br>${ship.vessel_type || ''}`, {
      ...
      >
        ...
        {darkAlerts.map((alert, idx) => (
          <Marker
            key={`dark-alt-${idx}`}
            position={[
              typeof alert?.lat === 'number' ? alert.lat : null,
              typeof alert?.lng === 'number' ? alert.lng : null
            ]}
            icon={darkIcon}
            eventHandlers={{
              click: () => {
        ...
        {spoofAlerts.map((alert, idx) => {
          const aisLat = typeof alert?.ais_lat === 'number' ? alert.ais_lat : null
          const aisLon = typeof alert?.ais_lon === 'number' ? alert.ais_lon : null
          const sarLat = typeof alert?.sar_lat === 'number' ? alert.sar_lat : null
          const sarLon = typeof alert?.sar_lon === 'number' ? alert.sar_lon : null
          // 🚨 Skip invalid data (prevents crash)
          if (aisLat === null || aisLon === null || sarLat === null || sarLon === null) {
            return null
          }
          return (
          <LayerGroup key={`spoof-group-${idx}`}>
            <Marker position={[aisLat, aisLon]} icon={aisIcon}>
              <Tooltip>AIS Reported Position</Tooltip>
              </Marker>
              
              <Marker position={[sarLat, sarLon]} icon={sarIconStatic}>
                <Tooltip>Actual SAR Detection</Tooltip>
                </Marker>
                <Polyline
                positions={[[aisLat, aisLon], [sarLat, sarLon]]}
                pathOptions={{ color: 'orange', dashArray: '6 4', weight: 2 }}
                >
                  <Tooltip sticky>
                    Deviation: {alert?.delta_km || 0} km
                    </Tooltip>
                    </Polyline>
                    </LayerGroup>
                    )
