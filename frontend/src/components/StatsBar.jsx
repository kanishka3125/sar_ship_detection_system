...
export default function StatsBar({ ships, loiteringCount = 0 }) {
  ...
  console.log("StatsBar ships:", safeShips)

  const total   = safeShips.length
  const high    = safeShips.filter(s => s.risk === 'HIGH').length
  const medium  = safeShips.filter(s => s.risk === 'MEDIUM').length
  const low     = safeShips.filter(s => s.risk === 'LOW').length
  const dark    = safeShips.filter(s => s.ais_status === 'ABSENT').length
  const spoofed = safeShips.filter(s => s.ais_status === 'SPOOFED').length
  const present = safeShips.filter(s => s.ais_status === 'PRESENT').length

  const avgConf = safeShips.length
    ? Math.round(
        safeShips.reduce((a, s) => a + (s.confidence || 0), 0) 
        / safeShips.length * 100
      )
    : 0
