      ...
      <div style={{
          ...
          {/* Panel header */}
            ...
            <div>
              ...
              <div style={{ fontSize: 9, color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)', marginTop: 2, opacity: 0.8 }}>
                {(ship.vessel_type || 'UNKNOWN').toUpperCase()} · {ship.length_m || '--'}M
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        <div style={{ flex: '0 0 40%', overflowY: 'auto', display: 'flex', flexDirection: 'column', background: 'var(--bg-card)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
              <div>
                <div style={{ fontSize: 20, fontWeight: 600, color: 'var(--text-primary)', letterSpacing: 0.5, lineHeight: 1.1 }}>
                  {ship.vessel_name || 'Unknown Vessel'}
                </div>
          ...
          <div style={{ padding: '18px 22px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, flex: 1, background: 'var(--bg-card)' }}>
            <Field label="VESSEL TYPE" value={ship.vessel_type || 'Unknown'} />
            <Field label="FLAG STATE" value={ship.flag || '--'} />
            <Field label="LENGTH" value={`${ship.length_m || '--'} m`} mono />
            <Field label="SPEED" value={`${ship.speed_knots || 0} kts`} mono />
            <Field label="HEADING" value={`${ship.heading || 0}°`} mono />
            <Field label="AIS STATUS" value={ship.ais_status || 'UNKNOWN'} color={aisColor} mono />
            ...
            label="COORDINATES" 
            value={`${
              typeof ship.lat === 'number' ? ship.lat.toFixed(4) : '--'
            }°N  ${
              typeof ship.lng === 'number' ? ship.lng.toFixed(4) : '--'
            }°E`} 
            mono 
            />
            <Field 
            label="LAST SEEN" 
            value={formatLocalTime(ship.last_seen || new Date().toISOString())} 
            mono 
            />
          </div>
          ...
          <div style={{ margin: '0 22px 18px' }}>
            <ConfidenceBar value={ship.confidence || 0} />
          </div>
        </div>
      </div>
    </div>
