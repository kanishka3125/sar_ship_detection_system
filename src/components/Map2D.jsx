...
function makeShipIcon(ship) {
  ...
  const pulseRings = isHigh ? `
    <div style="
      ...
      width:${size * 3.0}px;height:${size * 3.0}px;
      border-radius:50%;border:2px solid ${color};
      transform:translate(-50%,-50%);
      animation:markerPulse1 1.6s ease-out infinite;
      opacity:0.7;
    "></div>
    <div style="
      position:absolute;top:50%;left:50%;
      ...
      border-radius:50%;border:2px solid ${color};
      transform:translate(-50%,-50%);
      animation:markerPulse1 1.6s ease-out infinite 0.6s;
      opacity:0.5;
    "></div>
  ` : isMed ? `
    <div style="
      ...
      transform:translate(-50%,-50%);
      animation:markerPulse2 2.4s ease-out infinite;
      opacity:0.5;
    "></div>
  ` : ''
  ...
  const innerShape = isHigh ? `
    <div style="
      ...
      box-shadow:0 0 ${size}px ${color},0 0 ${size * 2.2}px ${glow};
    "></div>
  ` : `
    <div style="
      position:absolute;top:50%;left:50%;
      ...
      border-radius:50%;
      transform:translate(-50%,-50%);
      box-shadow:0 0 ${size}px ${color},0 0 ${size * 1.6}px ${glow};
    "></div>
  `
...
function InnerMap({ ships, onSelectShip, focusShip }) {
  ...
  useEffect(() => {
    ...
    validShips.forEach(ship => {
      ...
      popupContent.style.cssText = 'font-family:Space Grotesk,sans-serif;min-width:210px;'
      popupContent.innerHTML = `
        <div style="background:#08111f;border:1px solid ${color}44;border-radius:9px;padding:12px;color:#e8f0fe;">
          <div style="color:${color};font-weight:700;font-size:14px;margin-bottom:3px;">${ship.vessel_name}</div>
          <div style="color:#7a95b8;font-size:10px;font-family:JetBrains Mono,monospace;margin-bottom:8px;">${ship.id} · MMSI ${ship.mmsi}</div>
          ...
          <div style="font-size:10px;color:#7a95b8;margin-bottom:6px;line-height:1.4;">${ship.alert_reason.slice(0, 80)}…</div>
          <div style="font-size:9px;color:#7a95b8;font-family:JetBrains Mono,monospace;">
            ${lat.toFixed(4)}°N  ${lng.toFixed(4)}°E | ${ship.speed_knots}kt | HDG ${ship.heading}°
          </div>
        </div>
      `
      ...
      btn.style.cssText = `
        ...
        font-family:Space Grotesk,sans-serif;letter-spacing:0.5px;margin-top:6px;
        background:${color}22;color:${color};border:1px solid ${color}44;transition:background 0.2s;
      `
      ...
      marker.bindPopup(L.popup({ maxWidth: 260, className: 'zenith-popup', closeButton: true }).setContent(popupContent))
      ...
      >
        ...
        {ZONES.map((zone, i) => (
          <Polygon
            ...
            pathOptions={{ color: zone.color, fillColor: zone.color, fillOpacity: 0.09, weight: 1.5, dashArray: '6,4' }}
          >
            <Popup>
              <div style={{ background: '#08111f', padding: '8px 12px', borderRadius: '6px', color: zone.color, fontFamily: 'Space Grotesk,sans-serif', fontSize: '12px', fontWeight: 700 }}>
                ⚠ {zone.name}
              </div>
            </Popup>
          </Polygon>
